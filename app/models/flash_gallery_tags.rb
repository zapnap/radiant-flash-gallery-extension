module FlashGalleryTags
  ...
  tag 'gallery' do |tag|
    if tag.attr['name'] && gallery = Gallery.find_by_title(tag.attr['name'])
      ...
      out += "  <param name='FlashVars' value='xmlfile=#{gallery.xml_file_name}&xmlfiletype=Default' />"
      out += "  <embed src='#{gallery.swf_file_name}' quality='high' pluginspage='http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash' type='application/x-shockwave-flash' menu='false' FlashVars='xmlfile=#{gallery.xml_file_name}&xmlfiletype=Default'></embed>"
      out += "</object>"
    else
