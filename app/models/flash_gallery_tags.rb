module FlashGalleryTags
  ...
  tag 'gallery' do |tag|
    if tag.attr['name'] && gallery = Gallery.find_by_title(tag.attr['name'])
      ...
      out += "<embed src='#{gallery.swf.url}' quality='high' pluginspage='http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash' type='application/x-shockwave-flash' menu='false' FlashVars='xmlFilePath=#{gallery.xml_file_name}'></embed>"
      out += "</object>"
    else
